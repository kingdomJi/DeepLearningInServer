...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)#该函数是用来进行转置卷积的
            self.conv = DoubleConv(in_channels, out_channels)#这里是两次卷积，两次卷积之后的输出维度是out_channels

    def forward(self, x1, x2):#传入两种维度的特征图，例如1024和512，x1比x2维度高，但长宽窄
