...
def augmentation():#针对单张图片的增强方法
    ...
    trans = A.Compose(  # 创建流水线
        [
            ...
            # 在图像上生成矩形区域。mask随img变
            # A.OneOf(
            #     [
            #         A.CoarseDropout(max_holes=num, max_height=10, max_width=10, min_holes=None, min_height=None,
            #                         min_width=None,
            #                         fill_value=[40, 83, 77], mask_fill_value=0, always_apply=False, p=1),
            #         A.CoarseDropout(max_holes=num, max_height=10, max_width=10, min_holes=None, min_height=None,
            #                         min_width=None,
            #                         fill_value=[128,178,194], mask_fill_value=0, always_apply=False, p=1),
            #     ]
            # ,p=1),
...
def aug_seg(open_path_souce,open_path_label_seg,save_path_source,save_path_label_line,trans,suffix):#传入source地址，保存地址，数据增强流水线实例,该次的增强标记
    '''
    ...
    for i in range(len(mask_seg_list)):  # 读取文件夹下每一个图片
        image_souce = cv2.imread(open_path_souce + img_list[i])
        mask = cv2.imread(open_path_label_seg + mask_seg_list[i],cv2.IMREAD_GRAYSCALE)#读灰度图
        # transformed = transform(image=image)
        # tranformed_image = transformed['image']#将图片数据传递给transform（很明显这是个可调用的对象）的image参数，它会返回一个处理完的对象，
        transformed = trans(image=image_souce, mask=mask)
        transformed_image = transformed['image']
...
if __name__=='__main__':
    ##test查看单张增强效果
    # image_path = r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\test_img\public_Crack1.jpg'
    # save_path=r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\test_img\public_Crack1_resize.png'
    # trans_tool = transforms.Resize(256, 256)  # 设定筛选器图像器大小
    # img_resize = trans_tool(img)
    # trans = augmentation()
    # print(aug_test(image_path,save_path,trans))
    ########################resize
    # path_img = r'E:\jiangshan\U-net\Pytorch-UNet\data\crack_segmentation_dataset\images'
    # path_save = r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\aug6_seg\sum_publicAndkq6_img'
    # resize(path_img, path_save, mask=False, size=(512, 512))
    #########单张resize
    path_img = r'E:\jiangshan\U-net\Pytorch-UNet\data\DJ\DJI_20220731124015_0299.JPG'
    path_save = r'E:\jiangshan\U-net\Pytorch-UNet\data\DJ\DJI_20220731124015_0299_resize.JPG'
    ...
    # save_path_source = r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\aug5_2_seg\kq6_dom_aug\\'
    # save_path_label_seg = r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\aug5_2_seg\kq6_label_seg_aug\\'
    # # save_path_label_line = r'E:\jiangshan\U-net\Pytorch-UNet\data\data_Chen_new\augmentation_Jiang\patches\kq6_label_line_aug\\'
    # # trans = augmentation()
    # # suffix='aug_4'
    # # aug_seg(open_path_souce, open_path_label_seg, save_path_source, save_path_label_seg, trans, suffix)
